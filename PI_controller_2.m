...
while t_current < t_max
    ...
    % PI szabályozó: u = Kp*error + Ki*Ts*integrált hiba + Ki*Ts*mostani
    % hiba
    u = Kp *(error_val + Ki * dt_controller*(e_int + error_val));
    % Szaturáció
    int_flag = 1; %van-e telítés
    if u>Vmax
        u=Vmax;
        int_flag = 0; %telítés, integráló fagyasztása
    elseif u<-Vmax
        u=-Vmax;
        int_flag=0;     %telítés, integráló fagyasztása
    end
    %u = max(-Vmax, min(u, Vmax));
    if int_flag %ha nem volt telítés, akkor kumummláljuk az integráló tagot
    ...
    error_int_all = [error_int_all; repmat(e_int, length(T_temp), 1)];
...
xlabel('Idő [s]');
ylabel('Hiba [rad/s]');
title('DC motor szögsebesség hiba');
grid on;
